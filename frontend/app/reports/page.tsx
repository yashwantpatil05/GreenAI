...
export default function ReportsPage() {
  ...
  const projectsQ = useQuery({
    ...
    queryFn: async () => {
      const res = await apiFetch<{ items: Project[]; pagination: any }>("/projects/", {}, { token: token || undefined });
      return res?.items ?? [];
    },
  });
  ...
  const reportsQ = useQuery({
    ...
    enabled: Boolean(token),
    queryFn: async () => {
      const res = await apiFetch<{ items: Report[]; pagination: any }>("/reports/", {}, { token: token || undefined });
      return res?.items ?? [];
    },
  });
            ...
            <div className="flex items-center justify-end gap-2">
              <button
                ...
                onClick={async () => {
                  if (!token) return;
                  try {
                    const url = `/api/reports/${row.id}/download`;
                    ...
                    // Check if it's a redirect (GCS signed URL)
                ...
                className="inline-flex items-center justify-center rounded-lg border border-border/60 bg-background px-3 py-1.5 text-sm font-medium text-foreground hover:bg-accent"
