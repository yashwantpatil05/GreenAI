...
def summary(db: Session = Depends(get_db), user=Depends(get_current_user)):
    ...
    cache = get_cache()
    cache_key = cache.analytics_summary_key(str(user.organization_id))
    ...
    result = {
        ...
        "last_30_days": [
            {"day": str(row.day), "runs": row.runs, "energy_kwh": float(row.kwh), "co2e_kg": float(row.co2e)}
            for row in trend
        ],
    }
...
def trends(db: Session = Depends(get_db), user=Depends(get_current_user)):
    ...
    results = (
        db.query(
            ...
            func.sum(JobRunEnergy.total_kwh).label("total_kwh"),
            func.sum(JobRunEnergy.emissions_kg).label("total_emissions_kg"),
        )
        .join(JobRunEnergy.job_run)
        .join(JobRun.project)
        .filter(JobRun.project.has(organization_id=user.organization_id))
    ...
    return {
        "monthly_trends": [
            {
                "month": str(r.month) if r.month else None,
...
def hotspots(db: Session = Depends(get_db), user=Depends(get_current_user)):
    ...
    return {
        "hotspots": [
            {
                "run_name": r.run_name,
                "energy_kwh": float(r.total_kwh or 0),
