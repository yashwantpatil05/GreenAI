...
from backend.app.schemas.auth import Token, ForgotPasswordRequest, MessageResponse
...
def signup(payload: UserCreate, db: Session = Depends(get_db), ctx=Depends(get_request_context)):
    ...
    try:
        ...
        try:
            email_service.send_welcome_email(payload.email, org.name)
...
def forgot_password(payload: ForgotPasswordRequest):
    ...
    The reset link will redirect to {FRONTEND_URL}/reset-password.
    """
    base = _supabase_url()
    anon_key = _supabase_anon_key_fallback()
    ...
    try:
        ...
        logger.warning("Supabase recover request failed (ignored for security)")
