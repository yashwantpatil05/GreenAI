...
class RateLimiter:
    ...
    def _pg_check(self, db: Session, key: str, limit: int, burst: int) -> RateLimitResult:
        ...
        try:
            ...
            db.rollback()
            # Gracefully skip rate limiting if the table doesn't exist (e.g. test environments)
            if "no such table" in str(exc).lower() or "rate_limit_counters" in str(exc).lower():
                return RateLimitResult(limit=limit, remaining=limit, reset_seconds=60, blocked=False)
            raise
