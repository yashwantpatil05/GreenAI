...
def generate_project_report(db: Session, project: Project, from_date: Optional[datetime] = None, to_date: Optional[datetime] = None) -> Report:
    ...
    # Upload to storage (GCS or local storage)
    try:
        blob_name = f"{project.organization_id}/{filename}"
        storage_path = storage_service.upload_file(
            file_path=path,
            destination_blob_name=blob_name,
            content_type="application/pdf"
        )
        logger.info(f"Report uploaded to storage: {storage_path}")

        # Clean up local file if using GCS (keep local file in local mode)
        if not storage_service.local_mode:
            os.remove(path)
            logger.info(f"Local file cleaned up: {path}")
    except Exception as e:
        logger.error(f"Failed to upload report to storage: {e}")
...
def generate_job_run_report(db: Session, run: JobRun) -> Report:
    ...
    # Upload to storage (GCS or local storage)
    try:
        blob_name = f"{run.organization_id}/{filename}"
        storage_path = storage_service.upload_file(
            file_path=path,
            destination_blob_name=blob_name,
            content_type="application/pdf"
